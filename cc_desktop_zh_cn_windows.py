...
def patch_hardcoded_frontend_strings(app_dir: Path) -> None:
    ...
    replacements = {
        ...
        'description:"Total input+output tokens permitted per window before further messages are refused. Unset = no cap."': 'description:"每个窗口允许的输入+输出总令牌数，超过后会拒绝后续消息。未设置表示无上限。"',
        'description:"Tumbling window length for the token cap. Max 720 hours (30 days). The counter resets at the end of each window."': 'description:"令牌上限的滚动窗口长度。最大 720 小时（30 天）。计数器会在每个窗口结束时重置。"',
